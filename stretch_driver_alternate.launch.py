# to launch just python3 stretch_driver_alternate.py
# for args do like:
#  python3 stretch_driver_alternate.py --ros-args -p rate:=30.0 -p timeout:=0.5 -p controller_calibration_file:=/path/to/calibrated_controller.yaml -p broadcast_odom_tf:=True -p fail_out_of_range_goal:=True -p mode:=position
...
from launch_ros.substitutions import ThisLaunchFileDir
...
def generate_launch_description():
    ...
    alternate_driver_path = PathJoinSubstitution(
        [ThisLaunchFileDir(), 'stretch_driver_alternate.py']
    )
    stretch_driver = ExecuteProcess(
        cmd=[
            ...
            LaunchConfiguration('calibrated_controller_yaml_file'),
            '-p',
            'broadcast_odom_tf:=',
            LaunchConfiguration('broadcast_odom_tf'),
            '-p',
            'fail_out_of_range_goal:=',
            LaunchConfiguration('fail_out_of_range_goal'),
            '-p',
            'mode:=',
